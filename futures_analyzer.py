...
class FuturesDataManager:
    ...
    def fetch_price_data(self, trade_date: str, progress_callback=None) -> pd.DataFrame:
        ...
        # 添加广期所到行情数据交易所列表，并设置超时
        price_exchanges_with_timeout = []
        for exchange in self.price_exchanges:
            exchange_with_timeout = exchange.copy()
            if exchange['name'] == '广期所':
                exchange_with_timeout['timeout'] = 20  # 广期所使用更短超时
            else:
                exchange_with_timeout['timeout'] = 30
            price_exchanges_with_timeout.append(exchange_with_timeout)
        
        for i, exchange in enumerate(price_exchanges_with_timeout):
            try:
                if progress_callback:
                    progress_callback(f"正在获取{exchange['name']}行情数据...", 0.6 + (i / len(price_exchanges_with_timeout)) * 0.2)
                ...
                if exchange['name'] == '广期所':
                    ...
                    try:
                        ...
                        print(f"⚠️ 广期所行情数据获取失败，已自动跳过: {str(e)}")
                        continue
                else:
                    # 其他交易所使用正常的获取方式
                ...
                if df is not None and not df.empty:
                    ...
                    if exchange['name'] == '广期所':
                        print(f"⚠️ {exchange['name']} 行情数据为空，已自动跳过")
                    else:
                        print(f"⚠️ {exchange['name']} 行情数据为空")
                ...
                if exchange['name'] == '广期所':
                    print(f"⚠️ {exchange['name']} 行情数据获取失败，已自动跳过: {error_msg}")
                else:
                    print(f"获取{exchange['name']}行情数据失败: {error_msg}")
                continue
        ...
        # 显示获取结果统计
        total_exchanges = len(price_exchanges_with_timeout)
        if success_count >= 3:
